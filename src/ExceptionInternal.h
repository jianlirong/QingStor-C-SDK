 ...
 */
#ifndef _QINGSTOR_LIBQINGSTOR_EXCEPTIONINTERNAL_H_
#define _QINGSTOR_LIBQINGSTOR_EXCEPTIONINTERNAL_H_
    ...
    if (!nested) {
        boost::throw_exception(
            THROWABLE(buffer.c_str(), SkipPathPrefix(f), l,
                      QingStor::Internal::PrintStack(1, STACK_DEPTH).c_str()));
    } else {
        QingStor::throw_with_nested(
            THROWABLE(buffer.c_str(), SkipPathPrefix(f), l,
                      QingStor::Internal::PrintStack(1, STACK_DEPTH).c_str()));
    }
 ...
 * If this function return true, QingStorCanceled will be thrown.
 */
extern function<bool(void)> ChecnOperationCanceledCallback;

class QingStorException;
 ...
 * @throw return false if operation is not canceled, else throw QingStorCanceled.
 ...
 */
const char *GetExceptionDetail(const QingStor::QingStorException &e,
                               std::string &buffer);
...
#define THROW(throwable, fmt, ...) \
    QingStor::Internal::ThrowException<throwable>(false, __FILE__, __LINE__, #throwable, fmt, ##__VA_ARGS__);

#define NESTED_THROW(throwable, fmt, ...) \
    QingStor::Internal::ThrowException<throwable>(true, __FILE__, __LINE__, #throwable, fmt, ##__VA_ARGS__);

#endif /* _QINGSTOR_LIBQINGSTOR_EXCEPTIONINTERNAL_H_ */
